...
if exists_in_table("vps_plans", "name", "0") == -1:
    print("ADD VPS_PLAN: 0")
    add_vps_plan(0, 0, 0, 0, 0, 0)
if exists_in_table("vps_plans", "name", "1") == -1:
    print("ADD VPS_PLAN: 1")
    add_vps_plan(1, 2, 2, 80, 200, 1100000)
if exists_in_table("vps_plans", "name", "2") == -1:
    print("ADD VPS_PLAN: 2")
    add_vps_plan(2, 4, 4, 120, 300, 1900000)
if exists_in_table("vps_plans", "name", "3") == -1:
    print("ADD VPS_PLAN: 3")
    add_vps_plan(3, 6, 6, 160, 400, 2700000)
if exists_in_table("vps_plans", "name", "4") == -1:
    print("ADD VPS_PLAN: 4")
    add_vps_plan(4, 8, 8, 240, 500, 3700000)
...
for i in range(2, sheet.nrows):
    ...
    u_tel = sheet.cell_value(i, 4)
    u_mobile = sheet.cell_value(i, 5)
    u_email = sheet.cell_value(i, 6)
    u_username = sheet.cell_value(i, 9)
    u_nationalid = sheet.cell_value(i, 3)
    ...
    # company
    company_type = sheet.cell_value(i, 7)
    if company_type == 'دانشگاهی':
        ...
        company_type_int = '1'
    company_name = sheet.cell_value(i, 8)
    company_national_id = sheet.cell_value(i, 19)
    company_financial_id = sheet.cell_value(i, 20)
    company_registration_id = sheet.cell_value(i, 21)
    company_tel_number = sheet.cell_value(i, 11)

    # os-type
    os_type = sheet.cell_value(i, 26)

    # vps-plan
    vps_plan_id = exists_in_table("vps_plans", "name", str(int(sheet.cell_value(i, 33))))
