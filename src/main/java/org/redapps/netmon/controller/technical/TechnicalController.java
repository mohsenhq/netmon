...
public class TechnicalController {
        ...
        @Autowired
        public TechnicalController(ServiceIPRepository serviceIPRepository,
                        NetmonServiceRepository netmonServiceRepository, DeviceService colocationDeviceService,
                        ColocationDeviceRepository colocationDeviceRepository, OSTypeService osTypeService,
                        LogService logService, ServicePortRepository servicePortRepository,
                        ServiceTicketRepository serviceTicketRepository, IPService ipService, PortService portService,
                        TicketService ticketService, UserRepository userRepository, CompanyRepository companyRepository,
                        NSService nsService) {
        ...
        public String[] getTechnicalUrls() {
                ...
                return new String[] { "GET: /api/technical/urls", "GET: /api/technical/customers/all",
                                ...
                                "GET: /api/technical/plans/all", "POST: /api/technical/osType/new",
                                "PUT: /api/technical/osType/{osTypeId}/active",
                                "PUT: /api/technical/osType/{osTypeId}/inactive",
                                "PUT: /api/technical/osType/{osTypeId}/update", "GET: /api/technical/osType/all",
                                ...
                                "GET: /api/technical/colocations/all", "GET: /api/technical/colocations/{colocationId}",
                                "GET: /api/technical/vps/all", "GET: /api/technical/vps/{vpsId}" };
        }
         ...
         *                         the customer unique number who requested the service
        ...
        public ResponseEntity<?> createServiceIP(@Valid @RequestBody ServiceIPRequest serviceIPRequest,
                        @CurrentUser UserPrincipal currentUser, @PathVariable Long customerId,
                        @PathVariable String serviceType, @PathVariable Long serviceId,
                        @PathVariable String createDate) {

                initialChecking("CREATE_COLOCATION_IP", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + "]",
                                serviceIPRequest.toString());

                // Check if ip is already assigned to this service or not
                if (serviceIPRepository.existsByIpAndNetmonServiceIdAndNetmonServiceCreateDate(serviceIPRequest.getIp(),
                                serviceId, LocalDate.parse(createDate))) {
                        logService.createLog("CREATE_COLOCATION_IP", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",serviceId=" + serviceId + ",createDate="
                                                        + createDate + "]",
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{ipId}")
                                .buildAndExpand(serviceIp.getId()).toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "The IP created successfully."));
        }
        ...
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> deleteServiceIP(@CurrentUser UserPrincipal currentUser, @PathVariable Long customerId,
                        @PathVariable String serviceType, @PathVariable Long serviceId, @PathVariable String createDate,
                        @PathVariable Long ipId) {

                initialChecking("DELETE_SERVICE_IP", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + ",ipId=" + ipId + "]",
                                "");

                if (!serviceIPRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(ipId, serviceId,
                                LocalDate.parse(createDate))) {
                        logService.createLog("DELETE_SERVICE_IP", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",serviceId=" + serviceId + ",createDate="
                                                        + createDate + ",ipId=" + ipId + "]",
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{ipId}").buildAndExpand(ipId)
                                .toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "The IP deleted successfully."));
        ...
        public ResponseEntity<?> updateServiceIP(@Valid @RequestBody ServiceIPRequest serviceIPRequest,
                        @CurrentUser UserPrincipal currentUser, @PathVariable Long customerId,
                        @PathVariable String serviceType, @PathVariable Long ipId, @PathVariable Long serviceId,
                        @PathVariable String createDate) {

                initialChecking("UPDATE_SERVICE_IP", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + ",ipId=" + ipId + "]",
                                serviceIPRequest.toString());
                ...
                return ResponseEntity.created(location).body(new ApiResponse(true, "The IP updated successfully."));
        }

        /**
         * Create a new colocation device
         * 
         * @param colocationDeviceRequest the device information object the device
         *                                information object
         * @param currentUser             the user id who currently logged in
         * @param customerId              the customer unique number who requested the
         *                                service
         * @param colocationId            the unique colocation number
         * @param createDate              the service create Date
         * @return OK response or report error
         */
        @PostMapping("/customers/{customerId}/colocations/{colocationId}/{createDate}/devices/new")
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> createColocationDevice(@Valid @RequestBody DeviceRequest colocationDeviceRequest,
                ...
                initialChecking("UPDATE_SERVICE_IP", currentUser.getUsername(), customerId, "colocations", colocationId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",colocationId="
                                                + colocationId + ",createDate=" + createDate + "]",
                ...
                Device colocationDevice = colocationDeviceService.create(colocationDeviceRequest, currentUser,
                                colocationId, LocalDate.parse(createDate));

                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{deviceId}")
                                .buildAndExpand(colocationDevice.getId()).toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "The device created successfully."));
        }
         ...
         * @return OK response or report error
         */
        @DeleteMapping("/customers/{customerId}/colocations/{colocationId}/{createDate}/devices/{deviceId}/delete")
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> deleteColocationDevice(@CurrentUser UserPrincipal currentUser,
                        @PathVariable Long customerId, @PathVariable Long deviceId, @PathVariable Long colocationId,
                        @PathVariable String createDate) {

                initialChecking("UPDATE_SERVICE_IP", currentUser.getUsername(), customerId, "colocations", colocationId,
                                LocalDate.parse(createDate),
                                "[customerId=" + customerId + ",colocationId=" + colocationId + ",createDate="
                                                + createDate + ",deviceId=" + deviceId + "]",
                                "");

                if (!colocationDeviceRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(deviceId,
                                colocationId, LocalDate.parse(createDate))) {
                        logService.createLog("DELETE_COLOCATION_DEVICE", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",colocationId=" + colocationId + ",createDate="
                                                        + createDate + ",deviceId=" + deviceId + "]",
                ...
                return ResponseEntity.created(location).body(new ApiResponse(true, "The device deleted successfully."));
        }
         ...
         * @return OK response or report error
         */
        @PutMapping("/customers/{customerId}/colocations/{colocationId}/{createDate}/devices/{deviceId}/update")
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> updateColocationDevice(@Valid @RequestBody DeviceRequest deviceRequest,
                        @CurrentUser UserPrincipal currentUser, @PathVariable Long customerId,
                        @PathVariable Long deviceId, @PathVariable Long colocationId, @PathVariable String createDate) {
                ...
                if (!colocationDeviceRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(deviceId,
                                colocationId, LocalDate.parse(createDate))) {
                        logService.createLog("UPDATE_COLOCATION_DEVICE", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",colocationId=" + colocationId + ",createDate="
                                                        + createDate + ",deviceId=" + deviceId + "]",
                                        deviceRequest.toString(), "This device does not belong to the service.");
                        return new ResponseEntity<>(
                                        new ApiResponse(false, "This device does not belong to the service."),
                                        HttpStatus.BAD_REQUEST);
                }
                ...
                return ResponseEntity.created(location).body(new ApiResponse(true, "The device updated successfully."));
        }
         ...
         * @return OK response or report error
         */
        @PutMapping("/customers/{customerId}/colocations/{colocationId}/{createDate}/devices/{deviceId}/status")
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> changingColocationDeviceStatus(
                        @Valid @RequestBody DeviceStatusRequest deviceStatusRequest,
                        @CurrentUser UserPrincipal currentUser, @PathVariable Long customerId,
                        @PathVariable Long deviceId, @PathVariable Long colocationId, @PathVariable String createDate) {

                initialChecking("CHANGE_COLOCATION_DEVICE_STATUS", currentUser.getUsername(), customerId, "colocations",
                                colocationId, LocalDate.parse(createDate),
                                "[customerId=" + customerId + ",colocationId=" + colocationId + ",createDate="
                                                + createDate + ",deviceId=" + deviceId + "]",
                                deviceStatusRequest.toString());

                if (!colocationDeviceRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(deviceId,
                                colocationId, LocalDate.parse(createDate))) {
                        logService.createLog("CHANGE_COLOCATION_DEVICE_STATUS", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",colocationId=" + colocationId + ",createDate="
                                                        + createDate + ",deviceId=" + deviceId + "]",
         ...
         * @return OK response or report error
         */
        @PostMapping("/customers/{customerId}/{serviceType}/{serviceId}/{createDate}/ports/new")
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> createServicePort(@Valid @RequestBody ServicePortRequest servicePortRequest,
                ...
                initialChecking("CREATE_SERVICE_PORT", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + "]",
                                servicePortRequest.toString());

                NetmonService netmonService = netmonServiceRepository
                                .getOne(new ServiceIdentity(serviceId, LocalDate.parse(createDate)));
                if (servicePortRepository.existsByPortAndNetmonService(servicePortRequest.getPort(), netmonService)) {
                        logService.createLog("CREATE_SERVICE_PORT", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",serviceId=" + serviceId + ",createDate="
                                                        + createDate + "]",
                                        servicePortRequest.toString(), "This port is already assigned to the service.");
                        return new ResponseEntity<>(
                                        new ApiResponse(false, "This port is already assigned to the service."),
                                        HttpStatus.BAD_REQUEST);
                }
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{portId}")
                                .buildAndExpand(servicePort.getId()).toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "The port created successfully."));
        }
        ...
        public ResponseEntity<?> deleteServicePort(@CurrentUser UserPrincipal currentUser,
                        ...
                        @PathVariable String createDate, @PathVariable Long portId) {
                initialChecking("DELETE_SERVICE_PORT", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + ",portId=" + portId + "]",
                                "");

                if (!servicePortRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(portId, serviceId,
                                LocalDate.parse(createDate))) {
                        logService.createLog("DELETE_SERVICE_PORT", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",serviceId=" + serviceId + ",createDate="
                                                        + createDate + ",portId=" + portId + "]",
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{portId}").buildAndExpand(portId)
                                .toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "The port deleted successfully."));
         ...
         * @return OK response or report error
         */
        @PutMapping("/customers/{customerId}/{serviceType}/{serviceId}/{createDate}/ports/{portId}/update")
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> updateServicePort(@Valid @RequestBody ServicePortRequest servicePortRequest,
                        @CurrentUser UserPrincipal currentUser, @PathVariable Long customerId,
                        @PathVariable String serviceType, @PathVariable Long portId, @PathVariable Long serviceId,
                        @PathVariable String createDate) {
                initialChecking("UPDATE_SERVICE_PORT", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + ",portId=" + portId + "]",
                                servicePortRequest.toString());
                if (!servicePortRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(portId, serviceId,
                                LocalDate.parse(createDate))) {
                        logService.createLog("UPDATE_SERVICE_PORT", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",serviceId=" + serviceId + ",createDate="
                                                        + createDate + ",portId=" + portId + "]",
                ...
                return ResponseEntity.created(location).body(new ApiResponse(true, "The port updated successfully."));
        }
        ...
        public ResponseEntity<?> setTicketResponse(@Valid @RequestBody String response,
                ...
                initialChecking("SET_TICKET_RESPONSE", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + ",ticketId=" + ticketId + "]",
                                response);

                if (!serviceTicketRepository.existsByIdAndNetmonServiceIdAndNetmonServiceCreateDate(ticketId, serviceId,
                                LocalDate.parse(createDate))) {
                        logService.createLog("SET_TICKET_RESPONSE", currentUser.getUsername(),
                                        NetmonStatus.LOG_STATUS.FAILED,
                                        "[customerId=" + customerId + ",serviceId=" + serviceId + ",createDate="
                                                        + createDate + ",ticketId=" + ticketId + "]",
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{ticketId}")
                                .buildAndExpand(ticketId).toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "Setting response for ticket."));
        }
        ...
        public ResponseEntity<?> createOSType(@Valid @RequestBody OSTypeRequest osTypeRequest,
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{osTypeId}")
        ...
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> activeOsType(@PathVariable Long osTypeId, @CurrentUser UserPrincipal currentUser) {

                osTypeService.active(osTypeId, currentUser, true);

                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{planId}")
                                .buildAndExpand(osTypeId).toUri();

                return ResponseEntity.created(location).body(new ApiResponse(true, "The os-type active successfully."));
        }
        ...
        @PreAuthorize("hasRole('TECHNICAL')")
        public ResponseEntity<?> inactiveOsType(@PathVariable Long osTypeId, @CurrentUser UserPrincipal currentUser) {
         ...
         * 
         * @param currentUser the user id who currently logged in get username to log
         * @param page        the page number of the response (default value is 0) page
         *                    number
         * @param size        the page size of each response (default value is 30) page
         *                    size
        ...
        @PreAuthorize("hasRole('TECHNICAL')")
        public OSTypeResponse getOSTypeById(@CurrentUser UserPrincipal currentUser, @PathVariable Long osTypeId) {
        ...
        public ResponseEntity<?> updateOSTypeId(@Valid @RequestBody OSTypeRequest osTypeRequest,
                        @CurrentUser UserPrincipal currentUser, @PathVariable Long osTypeId) {
        ...
        public ResponseEntity<?> renameService(@Valid @RequestBody RenameServiceRequest renameServiceRequest,
                ...
                initialChecking("RENAME_SERVICE", currentUser.getUsername(), customerId, serviceType, serviceId,
                                LocalDate.parse(createDate), "[customerId=" + customerId + ",serviceId=" + serviceId
                                                + ",createDate=" + createDate + "]",
                ...
                URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{serviceId}/{createDate}")
         ...
         */
        private void initialChecking(String action, String username, Long customerId, String serviceType,
                        Long serviceId, LocalDate createDate, String requestParams, String bodyParams) {

                if (!userRepository.existsById(customerId)) {
                        logService.createLog(action, username, NetmonStatus.LOG_STATUS.FAILED, requestParams,
                                        bodyParams, "The customer does not exists.");
                ...
                if (!companyOptional.isPresent()) {
                        logService.createLog(action, username, NetmonStatus.LOG_STATUS.FAILED, requestParams,
                                        bodyParams, "The customer has no company.");
                ...
                if (!companyRepository.existsByIdAndUserId(companyId, customerId)) {
                        logService.createLog(action, username, NetmonStatus.LOG_STATUS.FAILED, requestParams,
                                        bodyParams, "This company does not belong to the customer.");
                ...
                if (!netmonServiceRepository.existsByIdAndCreateDateAndCompanyIdAndServiceType(serviceId, createDate,
                                companyId, srvType)) {
                        logService.createLog(action, username, NetmonStatus.LOG_STATUS.FAILED, requestParams,
                                        bodyParams, "This service does not belong to the company.");
