...
public class StaffController {
    ...
    public StaffController(TicketService ticketService, PortService portService,
                           ...
                           DeviceService colocationDeviceService, VpsPlanService vpsPlanService,
                           UserManagementService userManagementService, IPService ipService,
                           UserRepository userRepository, CompanyRepository companyRepository,
                           BillingService billingService, ServiceBillingRepository serviceBillingRepository) {
     ...
     * @param currentUser the user id who currently logged in the user id who currently logged in
    ...
    @PreAuthorize("hasRole('TECHNICAL') OR hasRole('MANAGER')")
    public PagedResponse<VpsResponse> getAllVPSes(@CurrentUser UserPrincipal currentUser,
                                                  @PathVariable Long customerId,
                                                  @RequestParam(value = "page",
                                                          defaultValue = AppConstants.DEFAULT_PAGE_NUMBER) int page,
                                                  @RequestParam(value = "size",
    ...
    public ServiceIPResponse getServiceIPById(@CurrentUser UserPrincipal currentUser,
        ...
        if(!companyOptional.isPresent()){
            logService.createLog("GET_SERVICE_IP_INFO", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + ",ipId=" + IPId + "]", "", "The customer has no company.");
            throw new ResourceNotFoundException("Company", "userId", customerId);
        }
        Long companyId = companyOptional.get().getId();
    ...
    public IpUsageResponse getServiceIpUsage(@RequestParam("startDate")
        ...
        if(!companyOptional.isPresent()){
            logService.createLog("GET_IP_USAGE", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + ",ipId=" + ipId + ",startDate=" +
                            startDate + ",endDate=" + endDate + "]", "", "The customer has no company.");
            throw new ResourceNotFoundException("Company", "userId", customerId);
        }
        Long companyId = companyOptional.get().getId();
        ...
        return new IpUsageResponse(startDate, endDate,
                ip.getIp(), ipUpUsages, ipDownUsages);
    }
    ...
    public PagedResponse<ColocationDeviceResponse> getColocationDevices(@CurrentUser UserPrincipal currentUser,
                                                                          ...
                                                                          @RequestParam(value = "size",
                                                                                  defaultValue = AppConstants.DEFAULT_PAGE_SIZE) int size) {
        if (!userRepository.existsById(customerId)) {
            logService.createLog("GET_ALL_COLOCATION_DEVICES", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",colocationId=" + colocationId + "]", "",
                    "The customer does not exists.");
            throw new ResourceNotFoundException("Customer", "customerId", customerId);
        }
        ...
        if(!companyOptional.isPresent()){
            logService.createLog("GET_ALL_COLOCATION_DEVICES", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",colocationId=" + colocationId + "]", "", "The customer has no company.");
            throw new ResourceNotFoundException("Company", "userId", customerId);
        }
        Long companyId = companyOptional.get().getId();
    ...
    public PagedResponse<ServicePortResponse> getServicePorts(@CurrentUser UserPrincipal currentUser,
        ...
        if (!netmonServiceRepository.existsByIdAndCompanyIdAndServiceType(serviceId, companyId, srvType)) {
            logService.createLog("GET_ALL_SERVICE_PORTS", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + "]", "", "This service does not belong to the company.");
            throw new AccessDeniedException("This service does not belong to the company.");
        }
    ...
    public ServicePortResponse getServicePortById(@CurrentUser UserPrincipal currentUser,
        ...
        if(!companyOptional.isPresent()){
            logService.createLog("GET_SERVICE_PORT_INFO", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + ",portId=" + portId + "]", "", "The customer has no company.");
            throw new ResourceNotFoundException("Company", "userId", customerId);
        }
        Long companyId = companyOptional.get().getId();
    ...
    public ServiceTicketResponse getServiceTicketById(@CurrentUser UserPrincipal currentUser,
        ...
        if(!companyOptional.isPresent()){
            logService.createLog("GET_SERVICE_TICKET_INFO", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + ",ticketId=" + ticketId + "]", "", "The customer has no company.");
            throw new ResourceNotFoundException("Company", "userId", customerId);
        }
        Long companyId = companyOptional.get().getId();
    ...
    public ResponseEntity<?> calculateBilling(@CurrentUser UserPrincipal currentUser,
        ...
        if (!userRepository.existsById(customerId)) {
            logService.createLog("CALCULATE_BILLING", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + ",startDate=" + startDate +
        ...
        if (!netmonServiceRepository.existsByIdAndCompanyIdAndServiceType(serviceId, companyId, srvType)) {
            logService.createLog("CALCULATE_BILLING", currentUser.getUsername(), NetmonStatus.LOG_STATUS.FAILED,
                    "[customerId=" + customerId + ",serviceId=" + serviceId + ",startDate=" + startDate +
                            ",endDate=" + endDate + "]", "", "This service does not belong to the company.");
            throw new AccessDeniedException("This service does not belong to the company.");
        }
